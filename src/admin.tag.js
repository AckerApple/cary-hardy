...
export const adminTag = tag(() => {
  ...
  const elmChangeDate = (event) => {
    ...
    config.nextMeetupDate = new Date(newDateString + ' ' + time).getTime()
  }

  const elmChangeTime = (event) => {
    const newTimeString = event.target.value
    config.nextMeetupDate = new Date(date + ' ' + newTimeString).getTime()
  }

  let qrUrl = letState('')(x => [qrUrl, qrUrl=x])
