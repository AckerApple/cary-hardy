...
import { content } from '../clock/clock.tag'
...
export const calendarLinksSection = tag(({
  nextMeetupDate,
  onQrUrlChange,
}: {
  ...
  calendarLinksSection.inputs((x) => {
    [{ nextMeetupDate, onQrUrlChange }] = x
    onQrUrlChange = output(onQrUrlChange)
  })

  const googleLink = getGoogleInviteLink({
    startDateTime: nextMeetupDate,
    ...
    div(
      ...
      div.style`display:flex;flex-wrap:wrap;gap:1em;flex-direction:column;`(
        div.style`display:flex;flex-grow:1`(
          ...
          button.onClick(() => onQrUrlChange(googleLink))('qr')
        ),
        div.style`display:flex;flex:1`(
          ...
          button.onClick(() => onQrUrlChange(outlookLink))('qr')
        ),
        div.style`display:flex;flex:1`(
